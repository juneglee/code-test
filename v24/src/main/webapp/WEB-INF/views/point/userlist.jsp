      ...
      <div class="info">
        <div class="info_lao">
         <img src="" alt="기본 이미지"><br>
          ...
          <input type="button" value="충전하기" onclick="showPopup();" />
                    
           <a id="withdraw_addform" class="btn btn-primary" href="#">충전하기</a>
           <!-- 팝업  -->
          
          <input type="button" id="naverPayBtn" value="충전하기 네이버페이 ">
          
          <p><a href="${pageContext.servletContext.contextPath}/WEB-INF/views/point/addform.jsp" onclick="popup(this.href,400,500,'팝업1','scroll'); return false;">충전 3</a></p>
          <!-- /. 팝업 -->
        </div>
      </div>
    </div>
    ...
    <div class="card text-white bg-secondary my-5 py-4 text-center">
      <div class="card-body">
        ...
        <p class="text-white m-0">기간 조회</p>
          <input id="startDate" type="text" name="date" size="12" />
          <input id="calendar" type="button" value="달력" onclick="$('#startDate').datepicker('show');" />

          <input id="endDate" type="text" name="date" size="12" />
          <input id="calendar" type="button" value="달력" onclick="$('#endDate').datepicker('show');" />
...
<input id='data-charge-name'  type="hidden" value='${loginUser.name}'/><br>
<input id="data-charge-price"  type="hidden" value='1000'/><br>
...
// -------------------  팝업1
  ...
  function showPopup() { 
	window.open(
			"${pageContext.servletContext.contextPath}/WEB-INF/point/withdrawAddform.html", 
...
document.querySelector("#withdraw_addform").onclick = () => {
	 BootPay.request({
		    ...
		    items: [
		      {
		          ...
		          price: 1000, //상품 단가
		          cat1: '1000', // 대표 상품의 카테고리 상, 50글자 이내
		          cat2: '5000', // 대표 상품의 카테고리 중, 50글자 이내
		          cat3: '10000', // 대표상품의 카테고리 하, 50글자 이내
		      }
		    ],
		    user_info: {
		      username: '송민호',
		   ...
		   // ------- 입력
		      addr: $("#data-charge-Addr"),
		   // ------- 입력
		      phone: $("#data-charge-tel")

		        // ------- 입력
		    },
		    order_id: '고유order_id_1234', //고유 주문번호로, 생성하신 값을 보내주셔야 합니다. // 생성 
		    params: {callback1: '그대로 콜백받을 변수 1', callback2: '그대로 콜백받을 변수 2', customvar1234: '변수명도 마음대로'},
		    account_expire_at: '2020-05-27', // 가상계좌 입금기간 제한 ( yyyy-mm-dd 포멧으로 입력해주세요. 가상계좌만 적용됩니다. )
		    extra: {
		      start_at: '2020-05-27', // 정기 결제 시작일 - 시작일을 지정하지 않으면 그 날 당일로부터 결제가 가능한 Billing key 지급
