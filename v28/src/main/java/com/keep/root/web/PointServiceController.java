...
import com.keep.root.domain.Pagination;
...
public class PointServiceController {
  ...
  @GetMapping("userlist")
  public void list(@ModelAttribute("Point") Point point,
	 ...
	 int listCnt = pointService.selectListCnt(point);
	 Pagination pagination = new Pagination(listCnt, curPage);
	 pagination.setPageSize(10);
	 
	 point.setStartIndex(pagination.getStartIndex());
	 point.setCntPerPage(pagination.getPageSize());
	 
	 model.addAttribute("listCnt", listCnt);
	 model.addAttribute("pagination", pagination);
	
    model.addAttribute("user", userService.get(loginUser.getNo()));
    model.addAttribute("userlist", pointService.list(point));
  }
